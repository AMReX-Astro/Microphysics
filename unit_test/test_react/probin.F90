...
module probin_module
  ...
  real (kind=rt), save, public :: dens_min = 1.d6
  !$acc declare create(dens_min)
  real (kind=rt), save, public :: dens_max = 1.d9
  !$acc declare create(dens_max)
  real (kind=rt), save, public :: temp_min = 1.d6
  !$acc declare create(temp_min)
  real (kind=rt), save, public :: temp_max = 1.d15
  !$acc declare create(temp_max)
  character (len=256), save, public :: xin_file = "uniform"
  !$acc declare create(xin_file)
  character (len=256), save, public :: run_prefix = ""
  !$acc declare create(run_prefix)
  character (len=256), save, public :: test_set = "gr0_3d"
  !$acc declare create(test_set)
  real (kind=rt), save, public :: tmax = 0.1d0
  !$acc declare create(tmax)
  ...
  integer, save, public :: do_acc = 1
  !$acc declare create(do_acc)
...
module runtime_init_module
  ...
  namelist /probin/ dens_min
  namelist /probin/ dens_max
  namelist /probin/ temp_min
  namelist /probin/ temp_max
  namelist /probin/ xin_file
  namelist /probin/ run_prefix
  namelist /probin/ test_set
  namelist /probin/ tmax
  namelist /probin/ small_temp
  namelist /probin/ small_dens
  namelist /probin/ do_acc
...
contains
  ...
  subroutine runtime_init(read_inputs_in)
    ...
    !$acc update &
    !$acc device(dens_min, dens_max, temp_min) &
    !$acc device(temp_max, xin_file, run_prefix) &
    !$acc device(test_set, tmax, small_temp) &
    !$acc device(small_dens, do_acc, use_eos_coulomb) &
    !$acc device(eos_input_is_constant, small_x, use_tables) &
...
103 format (1x, a3, 2x, a32, 1x, "=", 1x, l)
    write (unit,102) merge("   ", "[*]", dens_min == 1.d6), &
 "dens_min", dens_min
    write (unit,102) merge("   ", "[*]", dens_max == 1.d9), &
 "dens_max", dens_max
    write (unit,102) merge("   ", "[*]", temp_min == 1.d6), &
 "temp_min", temp_min
    write (unit,102) merge("   ", "[*]", temp_max == 1.d15), &
 "temp_max", temp_max
    write (unit,100) merge("   ", "[*]", xin_file == "uniform"), &
 "xin_file", trim(xin_file)
    write (unit,100) merge("   ", "[*]", run_prefix == ""), &
 "run_prefix", trim(run_prefix)
    write (unit,100) merge("   ", "[*]", test_set == "gr0_3d"), &
 "test_set", trim(test_set)
    write (unit,102) merge("   ", "[*]", tmax == 0.1d0), &
 "tmax", tmax
    write (unit,102) merge("   ", "[*]", small_temp == 1.e5), &
 "small_temp", small_temp
    write (unit,102) merge("   ", "[*]", small_dens == 1.e5), &
 "small_dens", small_dens
    write (unit,101) merge("   ", "[*]", do_acc == 1), &
 "do_acc", do_acc
