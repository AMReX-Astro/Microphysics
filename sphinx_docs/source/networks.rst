...
studies :raw-latex:`\cite{lowMach4}`. It includes a single-step
...
using the ideas from :raw-latex:`\cite{wallacewoosley:1981}`, but with modern
reaction rates from ReacLib :raw-latex:`\cite{ReacLib}` where available.
This network was used for the X-ray burst studies in
:raw-latex:`\cite{xrb:II,xrb:III}`, and more details are contained in those papers.
...
This is a 10 isotope network including rates from reactions suggested by Shen and Bildsten in their 2009 paper on helium burning on a white dwarf :raw-latex:`\cite{ShenBildsten}`.
...
and chemical equation `[chemeq:3.2] <#chemeq:3.2>`__ :raw-latex:`\cite{ShenBildsten}`.
...
-  burning_mode = 2 : hybrid approach
   ...
   This implements an approach from :raw-latex:`\cite{raskin:2010}` in which we do a
...
:raw-latex:`\cite{maestro:III}` for a discussion of the temperature evolution
equation.
...
This integrator is based on the stiff-ODE methods from :raw-latex:`\cite{NR}`, but
...
These correspond to the options presented in :raw-latex:`\cite{NR}`.
...
VODE is a classic integration package described in :raw-latex:`\cite{vode}`. We
