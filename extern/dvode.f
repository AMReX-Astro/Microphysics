...
C  20240424  eliminated computed GOTOs (in Blocks D and F).
C-----------------------------------------------------------------------
...
C Branch on KFLAG.  Note: Here KFLAG is 0, -1, or -2.
...
C ITASK = 3.  Jump to exit if TOUT was reached. ------------------------
        IF ((TN - TOUT)*H .GE. ZERO) GO TO 400
        GO TO 250
...
C Rescale the history array for a change in H by a factor of ETA. ------
 ...
 180    CONTINUE
      ...
      NQNYH = NQ*LDYH
      IF (LOGREJ .NE. 0) THEN
        WRITE(6,*) '[DVODE] REJECT_APPLY ETA=', ETA, ' -> H=', H
        WRITE(6,*) ' RC=', RC
        LOGREJ = 0
      ENDIF
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 ...
 220  CONTINUE
      ...
      RC = RC*(RL1/PRL1)
      PRL1 = RL1
      WRITE(6,*) '[DVODE] PRE tn=', TN, ' H=', H, ' NQ=', NQ, ' L=', L
      WRITE(6,*) ' RL1=', RL1, ' RC=', RC
      WRITE(6,*) ' NQWAIT=', NQWAIT, ' ETA=', ETA, ' TQ2=', TQ(2)
      WRITE(6,*) '[DVODE_CTL] PRE ETAMAX=', ETAMAX, ' HSCAL=', HSCAL,
     &           ' PRL1=', PRL1
      WRITE(6,*) '[DVODE_CTL] PRE CONP=', CONP, ' HMXI=', HMXI,
     &           ' NEWQ=', NEWQ, ' NEWH=', NEWH
      WRITE(6,*) '[DVODE_CTL] PRE IPUP=', IPUP, ' NSLJ=', NSLJ,
     &           ' NSLP=', NSLP, ' NST=', NST, ' NFE=', NFE,
     &           ' NJE=', NJE
      WRITE(6,*) '[DVODE_CTL] PRE EL=', EL(1), EL(2), EL(3), EL(4),
     &           EL(5), EL(6)
      WRITE(6,*) '[DVODE_CTL] PRE TAU=', TAU(1), TAU(2), TAU(3),
     &           TAU(4), TAU(5), TAU(6)
      WRITE(6,*) '[DVODE_CTL] PRE TQ=', TQ(1), TQ(2), TQ(3), TQ(4),
     &           TQ(5)
C
...
C
     ...
     &              ' RC=', RC, ' CRATE=', CRATE, ' NQWAIT=', NQWAIT
      ENDIF
        ...
        GO TO 450
      ENDIF
      WRITE(6,*) '[DVODE] CORR_FAIL ACNRM~', ACNRM
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      HU = H
      NQU = NQ
      WRITE(6,*) '[DVODE] ACCEPT t=', TN, ' hu=', HU, ' nq=', NQ
      DO 470 IBACK = 1, NQ
        I = L - IBACK
 470    TAU(I+1) = TAU(I)
      TAU(1) = H
 ...
 480    CONTINUE
     ...
     &           ' TQ5=', TQ(5), ' (pre-CONP update)'
      WRITE(6,*) '[DVODE_CTL] ACCEPT IPUP=', IPUP, ' NSLJ=', NSLJ,
     &           ' NSLP=', NSLP, ' NST=', NST, ' NFE=', NFE,
     &           ' NJE=', NJE
...
C Compute ratio of new H to current H at the current order. ------------
      ...
      WRITE(6,*) '[DVODE] REJECT DSM=', DSM, ' ETA=', ETA
      WRITE(6,*) ' NQWAIT=', NQWAIT, ' NEWQ=', NEWQ
      LOGREJ = 2
      GO TO 150
C-----------------------------------------------------------------------
...
C Compute ratio of new H to current H at current order plus one. -------
 ...
 575    SAVF(I) = ACOR(I) - CNQUOT*YH(I,LMAX)
      ...
      WRITE(6,*) '[DVODE] ORDER_DECIDE DSM=', DSM, ' ETAQ_eff=', ETAQ
      WRITE(6,*) ' ETAQM1=', ETAQM1, ' ETAQP1=', ETAQP1
...
C Test tentative new H against THRESH, ETAMAX, and HMXI, then exit. ----
 630  IF (ETA .LT. THRESH .OR. ETAMAX .EQ. ONE) GO TO 640
      ...
      NEWH = 1
      HNEW = H*ETA
      WRITE(6,*) '[DVODE] ORDER_APPLY ETA=', ETA, ' NEWQ=', NEWQ
      GO TO 690
 640  NEWQ = NQ
      NEWH = 0
      ETA = ONE
...
C-----------------------------------------------------------------------
 400  IF (M .NE. 0) CRATE = MAX(CRDOWN*CRATE,DEL/DELP)
      ...
      WRITE(6,*) ' RL1=', RL1, ' H=', H
      WRITE(6,*) '[DVODE_CTL] dvnlsd ICF=', ICF, ' IPUP=', IPUP,
     &           ' JCUR=', JCUR
...
C
      ...
      HRL1 = H*RL1
      WM(2) = HRL1
      IF (HRL1 .NE. PHRL1) THEN
      R = HRL1/PHRL1
      DO 320 I = 1,N
        DI = ONE - R*(ONE - ONE/WM(I+2))
        IF (ABS(DI) .EQ. ZERO) THEN
	  IERSL = 1
	  RETURN
	  ENDIF
 320    WM(I+2) = ONE/DI
