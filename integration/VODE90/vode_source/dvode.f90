...
210 IF ((TN - TOUT)*H .LT. ZERO) GO TO 250
    ...
    T = TOUT
    GO TO 420
220 TP = TN - dvode_state % HU*(ONE + HUN*UROUND)
...
330    IERPJ = 1
    ...
    else if (JOK .EQ. -1 .AND. MITER .EQ. 5) then
...
450 NFLAG = 0
    ...
    IF (M .GT. 0) dvode_state % ACNRM = DVNORM (N, ACOR, EWT)
    RETURN
...
540 ETA = MAX(ETAMIN,HMIN/ABS(H))
    ...
    HSCAL = H
    TAU(1) = H
    CALL F (N, TN, dvode_state % Y, SAVF, RPAR, IPAR)
    NFE = NFE + 1
    do I = 1, N
       YH(I,2) = H*SAVF(I)
    end do
