...
contains
  ...
  subroutine actual_integrator(state_in, state_out, dt, time)
    ...
    dvode_state % atol(SEDEN)           = atol_enuc ! total energy
    dvode_state % atol(SEINT)           = atol_enuc ! internal energy

    dvode_state % rtol(SFS:SFS-1+nspec) = rtol_spec ! mass fractions
    dvode_state % rtol(SEDEN)           = rtol_enuc ! total energy
    dvode_state % rtol(SEINT)           = rtol_enuc ! internal energy
