...
contains
  ...
  subroutine sneut5(temp,den,abar,zbar, &
    ...
    ! declare the pass
    real(rt) :: temp,den,abar,zbar, &
                        snu,dsnudt,dsnudd,dsnuda,dsnudz

    ! local variables
    real(rt) :: spair,spairdt,spairdd,spairda,spairdz, &
    ...
    real(rt) :: t9,xl,xldt,xlp5,xl2,xl3,xl4,xl5,xl6,xl7,xl8,xl9, &
    ...
    ! plasma
    real(rt) :: gl2,gl2dt,gl12,gl32,gl72,gl6, &
                        ft,ftdt,fl,fldt,fxy,fxydt, &
                        flda,fldz,ftda,ftdz, &
                        fxyda,fxydz,gl2da,gl2dz

    ! photo
    real(rt) :: tau,taudt,cos1,cos2,cos3,cos4,cos5,sin1,sin2, &
                        sin3,sin4,sin5,last,xast, &
                        fphot,fphotdt,qphot,qphotdt, &
                        fphotda,fphotdz,qphotda,qphotdz

    ! brem
    real(rt) :: t8,t812,t832,t82,t83,t85,t86,t8m1,t8m2,t8m3,t8m5, &
                        t8m6, &
    ...
    ! recomb
    real(rt) :: nu,nudt,nu2,nu3,bigj,bigjdt,nuda,nudz,bigjda,bigjdz


    ! numerical constants
    real(rt), parameter :: fac1   = 5.0e0_rt * M_PI / 3.0e0_rt
    real(rt), parameter :: fac2   = 10.0e0_rt * M_PI
    real(rt), parameter :: fac3   = M_PI / 5.0e0_rt
    real(rt), parameter :: oneth  = 1.0e0_rt/3.0e0_rt
    real(rt), parameter :: twoth  = 2.0e0_rt/3.0e0_rt
    real(rt), parameter :: con1   = 1.0e0_rt/5.9302e0_rt
    real(rt), parameter :: sixth  = 1.0e0_rt/6.0e0_rt
    real(rt), parameter :: iln10  = 4.342944819032518e-1_rt
    ...
    real(rt), parameter :: theta  = 0.2319e0_rt
    real(rt), parameter :: xnufam = 3.0e0_rt
    real(rt), parameter :: cv     = 0.5e0_rt + 2.0e0_rt * theta
    real(rt), parameter :: cvp    = 1.0e0_rt - cv
    real(rt), parameter :: ca     = 0.5e0_rt
    real(rt), parameter :: cap    = 1.0e0_rt - ca
    real(rt), parameter :: tfac1  = cv*cv + ca*ca + (xnufam-1.0e0_rt) * (cvp*cvp+cap*cap)
    real(rt), parameter :: tfac2  = cv*cv - ca*ca + (xnufam-1.0e0_rt) * (cvp*cvp - cap*cap)
    real(rt), parameter :: tfac3  = tfac2/tfac1
    real(rt), parameter :: tfac4  = 0.5e0_rt * tfac1
    ...
    ! equation 4.11
    if (abs(xnum) .gt. 0.7e0_rt  .or.  xden .lt. 0.0e0_rt) then
       fxy   = 1.0e0_rt
       fxydt = 0.0e0_rt
       fxydz = 0.0e0_rt
       fxyda = 0.0e0_rt
       ...
       a1  = 0.39e0_rt - 1.25e0_rt*xnum - 0.35e0_rt*sin(4.5e0_rt*xnum)
       a2  = -1.25e0_rt - 4.5e0_rt*0.35e0_rt*cos(4.5e0_rt*xnum)

       b1  = 0.3e0_rt * exp(-1.0e0_rt*(4.5e0_rt*xnum + 0.9e0_rt)**2)
    ...
    if (temp .ge. 1.0e7_rt  .and. temp .lt. 1.0e8_rt) then
       tau  =  log10(temp * 1.0e-7_rt)
       cc   =  0.5654e0_rt + tau
       c00  =  1.008e11_rt
       c01  =  0.0e0_rt
       c02  =  0.0e0_rt
    ...
    else if (temp .ge. 1.0e8_rt  .and. temp .lt. 1.0e9_rt) then
       tau   =  log10(temp * 1.0e-8_rt)
       cc   =  1.5654e0_rt
       c00  =  9.889e10_rt
       c01  = -4.524e8_rt
       c02  = -6.088e6_rt
    ...
    cos1 = cos(fac1*tau)
    cos2 = cos(fac1*2.0e0_rt*tau)
    cos3 = cos(fac1*3.0e0_rt*tau)
    cos4 = cos(fac1*4.0e0_rt*tau)
    cos5 = cos(fac1*5.0e0_rt*tau)
    last = cos(fac2*tau)

    sin1 = sin(fac1*tau)
    sin2 = sin(fac1*2.0e0_rt*tau)
    sin3 = sin(fac1*3.0e0_rt*tau)
    sin4 = sin(fac1*4.0e0_rt*tau)
    sin5 = sin(fac1*5.0e0_rt*tau)
    xast = sin(fac2*tau)
    ...
    tfermi = 5.9302e9_rt*(sqrt(1.0e0_rt+1.018e0_rt*(den6*ye)**twoth)-1.0e0_rt)

    ! "weak" degenerate electrons only
    if (temp .gt. 0.3e0_rt * tfermi) then
       ...
       c00   = 7.75e5_rt*t832 + 247.0e0_rt*t8**(3.85e0_rt)
       dd00  = (1.5e0_rt*7.75e5_rt*t812 + 3.85e0_rt*247.0e0_rt*t8**(2.85e0_rt))*1.0e-8_rt

       c01   = 4.07e0_rt + 0.0240e0_rt * t8**(1.4e0_rt)
       dd01  = 1.4e0_rt*0.0240e0_rt*t8**(0.4e0_rt)*1.0e-8_rt

       c02   = 4.59e-5_rt * t8**(-0.110e0_rt)
       dd02  = -0.11e0_rt*4.59e-5_rt * t8**(-1.11e0_rt)*1.0e-8_rt
       ...
       dumda  = -dum*abari
       dumdz  = 0.5738e0_rt*2.0e0_rt*ye*t86*den
       ...
       dum   = 2.275e-1_rt * zbar * zbar*t8m1 * (den6*abari)**oneth
       dumdt = -dum*tempi
       dumda = -oneth*dum*abari
       dumdz = 2.0e0_rt*dum*zbari
    ...
    xnum   = 1.10520e8_rt * den * ye /(temp*sqrt(temp))
    ...
    if (nu .ge. -20.0_rt  .and.  nu .le. 10.0_rt) then
       ...
       sreco   = tfac6 * 2.649e-18_rt * ye * zbar**13 * den * bigj*a1
       srecodt = sreco*(bigjdt*a2 - z*(zetadt + nudt)*a1)
       srecoda = sreco*(-1.0e0_rt*abari + bigjda*a2 - z*(zetada+nuda)*a1)
       srecodz = sreco*(14.0e0_rt*zbari + bigjdz*a2 - z*(zetadz+nudz)*a1)
  ...
  function ifermi12(f) result(ifermi12r)
    ...
    if (f .lt. 4.0e0_rt) then
       ...
       ff = 1.0e0_rt/f**(1.0e0_rt/(1.0e0_rt + an))
       rn = ff + a2(m2)
       do i=m2-1,1,-1
          rn = rn*ff + a2(i)
       enddo
