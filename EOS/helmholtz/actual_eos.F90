...
contains
  ...
  subroutine apply_electrons(state)
    ...
    !..various differences
    xt  = max( (state % T - t(jat)) * dti_sav(jat), ZERO)
    ...
    dpepdd = max(state % y_e * dpepdd, ZERO)
    ...
    pele    = x * df_d
    dpepdt  = x * df_dt
    s       = dpepdd/state % y_e - 2.0_rt * din * df_d
#ifdef EXTRA_THERMO
    dpepda  = -ytot1 * (2.0_rt * pele + s * din)
    dpepdz  = state % rho*ytot1*(2.0_rt * din * df_d  +  s)
#endif
  ...
  subroutine apply_ions(state)
    ...
    real(rt), parameter :: pi      = 3.1415926535897932384_rt
    real(rt), parameter :: sioncon = (2.0_rt * pi * amu * kerg)/(h*h)
    ...
    eion    = 1.5_rt * pion*deni
    deiondd = (1.5_rt * dpiondd - eion)*deni
    deiondt = 1.5_rt * dpiondt*deni
#ifdef EXTRA_THERMO
    deionda = 1.5_rt * dpionda*deni
    deiondz = ZERO
#endif
  ...
  subroutine apply_radiation(state)
...
#ifdef RADIATION
    real(rt), parameter :: ssol    = ZERO
#else
    real(rt), parameter :: ssol    = 5.67051e-5_rt
#endif
    real(rt), parameter :: asol    = 4.0_rt * ssol / clight
    real(rt), parameter :: asoli3  = asol/3.0_rt
    ...
    dpraddd = ZERO
    dpraddt = 4.0_rt * prad*tempi
#ifdef EXTRA_THERMO
    dpradda = ZERO
    dpraddz = ZERO
#endif

    erad    = 3.0_rt * prad*deni
    deraddd = -erad*deni
    deraddt = 3.0_rt * dpraddt*deni
#ifdef EXTRA_THERMO
    deradda = ZERO
    deraddz = ZERO
#endif
  ...
  subroutine apply_coulomb_corrections(state)
    ...
    real(rt), parameter :: a1 = -0.898004_rt
    real(rt), parameter :: b1 =  0.96786_rt
    real(rt), parameter :: c1 =  0.220703_rt
    real(rt), parameter :: d1 = -0.86097_rt
    real(rt), parameter :: e1 =  2.5269_rt
    real(rt), parameter :: a2 =  0.29561_rt
    real(rt), parameter :: b2 =  1.9885_rt
    real(rt), parameter :: c2 =  0.288675_rt
    real(rt), parameter :: qe   = 4.8032042712e-10_rt
    real(rt), parameter :: esqu = qe * qe
    real(rt), parameter :: onethird = 1.0_rt/3.0_rt
    real(rt), parameter :: forth = 4.0_rt/3.0_rt
    ...
    plasgdz  = 2.0_rt * plasg/state % zbar

    !...yakovlev & shalybkov 1989 equations 82, 85, 86, 87
    if (plasg .ge. ONE) then
       x        = plasg**(0.25_rt)
       y        = avo_eos * ytot1 * kerg
       ecoul    = y * state % T * (a1*plasg + b1*x + c1/x + d1)
       pcoul    = onethird * state % rho * ecoul
       scoul    = -y * (3.0_rt*b1*x - 5.0_rt*c1/x &
                  + d1 * (log(plasg) - 1.0_rt) - e1)

       y        = avo_eos*ytot1*kt*(a1 + 0.25_rt/plasg*(b1*x - c1/x))
       decouldd = y * plasgdd
       ...
       pcoul    = -pion * z
       ecoul    = 3.0_rt * pcoul/state % rho
       scoul    = -avo_eos/state % abar*kerg*(c2*x -a2*(b2-1.0_rt)/b2*y)

       s        = 1.5_rt*c2*x/plasg - onethird*a2*b2*y/plasg
  ...
  pure function psi0(z) result(psi0r)
    ...
    real(rt) :: psi0r
    !$gpu
    psi0r = z**3 * ( z * (-6.0_rt*z + 15.0_rt) -10.0_rt) + 1.0_rt
  end function psi0
  ...
  pure function dpsi0(z) result(dpsi0r)
    ...
    real(rt) :: dpsi0r
    !$gpu
    dpsi0r = z**2 * ( z * (-30.0_rt*z + 60.0_rt) - 30.0_rt)
  end function dpsi0
  ...
  pure function ddpsi0(z) result(ddpsi0r)
    ...
    real(rt) :: ddpsi0r
    !$gpu
    ddpsi0r = z* ( z*( -120.0_rt*z + 180.0_rt) -60.0_rt)
  end function ddpsi0
  ...
  pure function psi1(z) result(psi1r)
    ...
    real(rt) :: psi1r
    !$gpu
    psi1r = z* ( z**2 * ( z * (-3.0_rt*z + 8.0_rt) - 6.0_rt) + 1.0_rt)
  end function psi1
  ...
  pure function dpsi1(z) result(dpsi1r)
    ...
    real(rt) :: dpsi1r
    !$gpu
    dpsi1r = z*z * ( z * (-15.0_rt*z + 32.0_rt) - 18.0_rt) +1.0_rt
  end function dpsi1
  ...
  pure function ddpsi1(z) result(ddpsi1r)
    ...
    real(rt) :: ddpsi1r
    !$gpu
    ddpsi1r = z * (z * (-60.0_rt*z + 96.0_rt) -36.0_rt)
  end function ddpsi1
  ...
  pure function psi2(z) result(psi2r)
    ...
    real(rt) :: psi2r
    !$gpu
    psi2r = 0.5_rt*z*z*( z* ( z * (-z + 3.0_rt) - 3.0_rt) + 1.0_rt)
  end function psi2
  ...
  pure function dpsi2(z) result(dpsi2r)
    ...
    real(rt) :: dpsi2r
    !$gpu
    dpsi2r = 0.5_rt*z*( z*(z*(-5.0_rt*z + 12.0_rt) - 9.0_rt) + 2.0_rt)
  end function dpsi2
  ...
  pure function ddpsi2(z) result(ddpsi2r)
    ...
    real(rt) :: ddpsi2r
    !$gpu
    ddpsi2r = 0.5_rt*(z*( z * (-20.0_rt*z + 36.0_rt) - 18.0_rt) + 2.0_rt)
  end function ddpsi2
  ...
  pure function xpsi0(z) result(xpsi0r)
    ...
    real(rt) :: xpsi0r
    !$gpu
    xpsi0r = z * z * (2.0_rt*z - 3.0_rt) + 1.0_rt
  end function xpsi0
  ...
  pure function xdpsi1(z) result(xdpsi1r)
    ...
    real(rt) :: xdpsi1r
    !$gpu
    xdpsi1r = z * (3.0_rt*z - 4.0_rt) + 1.0_rt
  end function xdpsi1
