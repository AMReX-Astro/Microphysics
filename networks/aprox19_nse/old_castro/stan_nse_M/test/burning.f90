...
!     read in table 
   open(unit=4,file='burning_table.dat',status='old',iostat=istat)
   if (istat > 0) then
      print *, 'EOSINIT: Failed to open burning_table.dat'
      stop
   endif
