      subroutine sneut5(temp,den,abar,zbar, &
        ...
        !$acc routine seq
        !$acc routine(ifermi12) seq
        !$acc routine(zfermim12) seq
...
! load the coefficients of the expansion
      ...
      m2 = 6
      k2 = 5
      a1(1:5) = [ 1.999266880833d4,   5.702479099336d3, &
                  6.610132843877d2,   3.818838129486d1, &
                  1.0d0 ]
      b1(1:4) = [ 1.771804140488d4,  -2.014785161019d3, &
                  9.130355392717d1,  -1.670718177489d0 ]
      a2(1:7) = [ -1.277060388085d-2,  7.187946804945d-2, &
                  -4.262314235106d-1,  4.997559426872d-1, &
                  -1.285579118012d0,  -3.930805454272d-1, &
                  1.0d0 ]
      b2(1:6) = [ -9.745794806288d-3,  5.485432756838d-2, &
...
! load the coefficients of the expansion
      ...
      b2(1:12) = [ -2.23310170962369d-15, -7.94193282071464d-13, &
