...
contains
  ...
  ! The actual_rhs routine provides the right-hand-side for the DVODE solver.
  ! It deals with molar abundances throughout (we expect that the input
  ! vector y has molar abundances, Y = X/A) for make_rates, and
  ! dydt routines.  It also computes the energy release.
  ...
  subroutine actual_rhs(neq, time, y, ydot, rpar)
    ...
    double precision :: dens, t9

    ydot = ZERO

    dens = rpar(irp_dens)
    t9   = y(net_itemp) / 10.0**9

    ! build the rates; weak rates are the wk* variables
    call make_rates(t9, dens, y(1:nspec), rpar)

    ! set up the ODEs for the species
    call make_ydots(y(1:nspec),t9,rpar,ydot(1:nspec),.false.)
  ...
  subroutine make_rates(t9,dens,y,rpar)
    ...
    rpar(irp_rates:irp_drtdt-1+nrates) = ZERO ! rates and dratesdt
  ...
  subroutine actual_jac(neq, time, y, pd, rpar)
    ...
    integer         , intent(IN   ) :: neq
    double precision, intent(IN   ) :: y(neq), time
    double precision, intent(INOUT) :: rpar(n_rpar_comps)
    ...
    call make_ydots(ymol,t9,rpar,pd(1:nspec,net_itemp),.true.)
