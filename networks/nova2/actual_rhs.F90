...
contains
  ...
  subroutine evaluate_rates(state, rate_eval)
    ...
    if (screen_reaclib) then
      ...
      rate_eval % unscreened_rates(i_scor,27) = scor
      rate_eval % unscreened_rates(i_dscor_dt,27) = dscor_dt


      call screen5(pstate, 20, scor, dscor_dt, dscor_dd)
      rate_eval % unscreened_rates(i_scor,28) = scor
      rate_eval % unscreened_rates(i_dscor_dt,28) = dscor_dt


      call screen5(pstate, 21, scor, dscor_dt, dscor_dd)
      rate_eval % unscreened_rates(i_scor,29) = scor
      rate_eval % unscreened_rates(i_dscor_dt,29) = dscor_dt


      call screen5(pstate, 22, scor, dscor_dt, dscor_dd)
      rate_eval % unscreened_rates(i_scor,30) = scor
      rate_eval % unscreened_rates(i_dscor_dt,30) = dscor_dt


      call screen5(pstate, 23, scor, dscor_dt, dscor_dd)

      call screen5(pstate, 24, scor2, dscor2_dt, dscor2_dd)
      rate_eval % unscreened_rates(i_scor,31) = scor * scor2
      rate_eval % unscreened_rates(i_dscor_dt,31) = scor * dscor2_dt + dscor_dt * scor2
  ...
  subroutine rhs_nuc(state, ydot_nuc, Y, screened_rates)
    ...
    ydot_nuc(jp) = ( &
      ...
      screened_rates(k_d_he3__p_he4)*Y(jd)*Y(jhe3)*state % rho + 2.0e0_rt* &
    ...
    ydot_nuc(jd) = ( &
      ...
      Y(jd)*Y(jhe3)*state % rho - screened_rates(k_p_d__he3)*Y(jd)*Y(jp)* &
      state % rho + 0.5e0_rt*screened_rates(k_p_p__d__weak__bet_pos_)*Y(jp)**2* &
      state % rho + 0.5e0_rt*screened_rates(k_p_p__d__weak__electron_capture)*Y(jp) &
      **2*state % rho**2*state % y_e &
       )
    ...
    ydot_nuc(jhe4) = ( &
      2.0e0_rt*screened_rates(k_b8__he4_he4__weak__wc12)*Y(jb8) + 2.0e0_rt* &
      screened_rates(k_d_be7__p_he4_he4)*Y(jbe7)*Y(jd)*state % rho + 0.5e0_rt* &
      screened_rates(k_d_d__he4)*Y(jd)**2*state % rho + screened_rates(k_d_he3__p_he4)* &
      Y(jd)*Y(jhe3)*state % rho + 2.0e0_rt*screened_rates(k_he3_be7__p_p_he4_he4)* &
      Y(jbe7)*Y(jhe3)*state % rho + 0.5e0_rt*screened_rates(k_he3_he3__p_p_he4)* &
      Y(jhe3)**2*state % rho - screened_rates(k_he4_c12__o16)*Y(jc12)*Y(jhe4) &
      *state % rho - screened_rates(k_he4_he3__be7)*Y(jhe3)*Y(jhe4)*state % rho - &
      0.5e0_rt*screened_rates(k_he4_he4_he4__c12)*Y(jhe4)**3*state % rho**2 - &
      screened_rates(k_he4_n13__p_o16)*Y(jhe4)*Y(jn13)*state % rho - &
    ...
    ydot_nuc(jb8) = ( &
      -screened_rates(k_b8__he4_he4__weak__wc12)*Y(jb8) + screened_rates(k_p_be7__b8)* &
      Y(jbe7)*Y(jp)*state % rho &
       )
  ...
  subroutine jac_nuc(state, jac, Y, screened_rates)
    ...
    scratch = (&
      -screened_rates(k_p_be7__b8)*Y(jbe7)*state % rho - screened_rates(k_p_c12__n13)*Y(jc12)* &
      state % rho - screened_rates(k_p_c13__n14)*Y(jc13)*state % rho - &
      screened_rates(k_p_d__he3)*Y(jd)*state % rho - screened_rates(k_p_f18__he4_o15)* &
      Y(jf18)*state % rho - screened_rates(k_p_he3__he4__weak__bet_pos_)* &
      Y(jhe3)*state % rho - screened_rates(k_p_n13__o14)*Y(jn13)*state % rho - &
      screened_rates(k_p_n14__o15)*Y(jn14)*state % rho - screened_rates(k_p_n15__he4_c12)* &
      Y(jn15)*state % rho - screened_rates(k_p_n15__o16)*Y(jn15)*state % rho - &
      screened_rates(k_p_o16__f17)*Y(jo16)*state % rho - screened_rates(k_p_o17__f18)* &
      Y(jo17)*state % rho - screened_rates(k_p_o17__he4_n14)*Y(jo17)*state % rho - &
      2.0e0_rt*screened_rates(k_p_p__d__weak__bet_pos_)*Y(jp)*state % rho - 2.0e0_rt* &
    ...
    scratch = (&
      screened_rates(k_d_he3__p_he4)*Y(jd)*state % rho + 2.0e0_rt* &
      screened_rates(k_he3_be7__p_p_he4_he4)*Y(jbe7)*state % rho + 2.0e0_rt* &
    ...
    scratch = (&
      screened_rates(k_d_be7__p_he4_he4)*Y(jd)*state % rho + 2.0e0_rt* &
      screened_rates(k_he3_be7__p_p_he4_he4)*Y(jhe3)*state % rho - &
      screened_rates(k_p_be7__b8)*Y(jp)*state % rho &
       )
    call set_jac_entry(jac, jp, jbe7, scratch)
    ...
    scratch = (&
      -screened_rates(k_p_d__he3)*Y(jd)*state % rho + 1.0e0_rt* &
      screened_rates(k_p_p__d__weak__bet_pos_)*Y(jp)*state % rho + 1.0e0_rt* &
      screened_rates(k_p_p__d__weak__electron_capture)*Y(jp)*state % rho**2* &
      state % y_e &
       )
    ...
    scratch = (&
      -screened_rates(k_d_be7__p_he4_he4)*Y(jbe7)*state % rho - 2.0e0_rt*screened_rates(k_d_d__he4)* &
    ...
    call set_jac_entry(jac, jd, jbe7, scratch)
    ...
    scratch = (&
      -screened_rates(k_d_he3__p_he4)*Y(jd)*state % rho - screened_rates(k_he3_be7__p_p_he4_he4)* &
      Y(jbe7)*state % rho - 2.0e0_rt*screened_rates(k_he3_he3__p_p_he4)*Y(jhe3)* &
      state % rho - screened_rates(k_he4_he3__be7)*Y(jhe4)*state % rho - &
      screened_rates(k_p_he3__he4__weak__bet_pos_)*Y(jp)*state % rho &
       )
    call set_jac_entry(jac, jhe3, jhe3, scratch)

    scratch = (&
      -screened_rates(k_he4_he3__be7)*Y(jhe3)*state % rho &
       )
    call set_jac_entry(jac, jhe3, jhe4, scratch)

    scratch = (&
      -screened_rates(k_he3_be7__p_p_he4_he4)*Y(jhe3)*state % rho &
       )
    call set_jac_entry(jac, jhe3, jbe7, scratch)
    ...
    scratch = (&
      2.0e0_rt*screened_rates(k_d_be7__p_he4_he4)*Y(jbe7)*state % rho + 1.0e0_rt* &
    ...
    scratch = (&
      screened_rates(k_d_he3__p_he4)*Y(jd)*state % rho + 2.0e0_rt* &
      screened_rates(k_he3_be7__p_p_he4_he4)*Y(jbe7)*state % rho + 1.0e0_rt* &
      screened_rates(k_he3_he3__p_p_he4)*Y(jhe3)*state % rho - &
    ...
    scratch = (&
      -screened_rates(k_he4_c12__o16)*Y(jc12)*state % rho - screened_rates(k_he4_he3__be7)* &
      Y(jhe3)*state % rho - 1.5e0_rt*screened_rates(k_he4_he4_he4__c12)*Y(jhe4)**2 &
      *state % rho**2 - screened_rates(k_he4_n13__p_o16)*Y(jn13)*state % rho - &
    ...
    scratch = (&
      2.0e0_rt*screened_rates(k_d_be7__p_he4_he4)*Y(jd)*state % rho + 2.0e0_rt* &
      screened_rates(k_he3_be7__p_p_he4_he4)*Y(jhe3)*state % rho &
       )
    call set_jac_entry(jac, jhe4, jbe7, scratch)

    scratch = (&
      2.0e0_rt*screened_rates(k_b8__he4_he4__weak__wc12) &
       )
    call set_jac_entry(jac, jhe4, jb8, scratch)
    ...
    call set_jac_entry(jac, jbe7, jp, scratch)

    scratch = (&
      -screened_rates(k_d_be7__p_he4_he4)*Y(jbe7)*state % rho &
       )
    call set_jac_entry(jac, jbe7, jd, scratch)

    scratch = (&
      -screened_rates(k_he3_be7__p_p_he4_he4)*Y(jbe7)*state % rho + &
      screened_rates(k_he4_he3__be7)*Y(jhe4)*state % rho &
       )
    call set_jac_entry(jac, jbe7, jhe3, scratch)

    scratch = (&
      screened_rates(k_he4_he3__be7)*Y(jhe3)*state % rho &
       )
    call set_jac_entry(jac, jbe7, jhe4, scratch)
    ...
    call set_jac_entry(jac, jbe7, jbe7, scratch)

    scratch = (&
      screened_rates(k_p_be7__b8)*Y(jbe7)*state % rho &
       )
    call set_jac_entry(jac, jb8, jp, scratch)

    scratch = (&
      screened_rates(k_p_be7__b8)*Y(jp)*state % rho &
       )
    call set_jac_entry(jac, jb8, jbe7, scratch)

    scratch = (&
      -screened_rates(k_b8__he4_he4__weak__wc12) &
       )
    call set_jac_entry(jac, jb8, jb8, scratch)
