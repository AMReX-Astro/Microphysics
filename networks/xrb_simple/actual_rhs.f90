...
contains
  ...
  subroutine actual_rhs(state)
    ...
    ! build the rates
    call make_rates(t9, dens, ymol, state)

    ! set up the ODEs
    call make_ydots(ymol, t9, state, ydot(1:nspec))
  ...
  subroutine make_rates(t9, dens, y, state)
    ...
    state % rates(1,ir3a) = 2.79d-8*t9m3*exp(-4.403_dp_t*t9m1)*dens*dens/SIX

    ! 15o(ag)19ne
    state % rates(1,irag15) = (19.0_dp_t * (t9**2.85_dp_t) * &
         exp(-7.356_dp_t*t9m1) + &
    ...
    state % rates(1,irap14) = 1.68d13*t9m23 * &
    ...
    state % rates(1,irap14) = dens*state % rates(1,irap14)

    ! 18ne(ap)21na
    state % rates(1,irap18) = exp(56.59577d0-2.006856d0*t9m1 &
         +26.05828d0*t9m13 &
         -87.88732d0*t913 + 3.718809d0*t9 - 0.1767444d0*t953 &
         + 46.971960d0*t9log)
    state % rates(1,irap18) = dens*state % rates(1,irap18)

    ! weak rates
    state % rates(1,irwk14o) = wk14o
    state % rates(1,irwk15o) = wk15o

  end subroutine make_rates



  subroutine make_ydots(ymol, t9, state, dydt)
    ...
    ! o14
    dydt(io14) = ymol(ihe4)**3 * state % rates(1,ir3a) &
         + ymol(io15) * state % rates(1,irwk15o)       &
         - ymol(io14) * ymol(ihe4) * state % rates(1,irap14)

    ! o15
    dydt(io15) = ymol(io14) * state % rates(1,irwk14o) &
         - ymol(io15) * ymol(ihe4) * state % rates(1,irag15) &
         - ymol(io15) * state % rates(1,irwk15o)

    ! ne18
    dydt(ine18) = ymol(io14) * ymol(ihe4) * state % rates(1,irap14) &
         - ymol(ine18) * ymol(ihe4) * state % rates(1,irap18)

    ! si25
    dydt(isi25) = ymol(io15) * ymol(ihe4) * state % rates(1,irag15) &
         + ymol(ine18) * ymol(ihe4) * state % rates(1,irap18)

    ! he4
    dydt(ihe4) = ymol(io15) * state % rates(1,irwk15o) &
         - THREE * ymol(ihe4)**3 * state % rates(1,ir3a) &
         - ymol(io14) * ymol(ihe4) * state % rates(1,irap14) &
         - ymol(io15) * ymol(ihe4) * state % rates(1,irag15) &
         - ymol(ine18) * ymol(ihe4) * state % rates(1,irap18)

    ! h1
    dydt(ih1) = - TWO * ymol(ihe4)**3 * state % rates(1,ir3a) &
         - SIX * ymol(io15) * ymol(ihe4) * state % rates(1,irag15) &
         - THREE * ymol(ine18) * ymol(ihe4) * state % rates(1,irap18) &
         - ymol(io14) * state % rates(1,irwk14o) &
         - THREE * ymol(io15) * state % rates(1,irwk15o)
