...
contains
  ...
  subroutine actual_rhs(state)
    ...
    scratch_8 = state%rates(i_rate, k_na23_ne23)*state%rates(i_scor, k_na23_ne23)*Y(jna23)
    scratch_9 = state%rates(i_rate, k_ne23_na23)*state%rates(i_scor, k_ne23_na23)*Y(jne23)
    ...
    state%ydot(net_ienuc) = state%ydot(net_ienuc) + N_AVO * state%ydot(jna23) * dqweak(j_na23_ne23)
    state%ydot(net_ienuc) = state%ydot(net_ienuc) + N_AVO * state%ydot(jne23) * dqweak(j_ne23_na23)
    
    ! weak particle energy generation rates from gamma heating and neutrino loss
    ! (does not include plasma neutrino losses)
    state%ydot(net_ienuc) = state%ydot(net_ienuc) + N_AVO * Y(jna23) * epart(j_na23_ne23)
    state%ydot(net_ienuc) = state%ydot(net_ienuc) + N_AVO * Y(jne23) * epart(j_ne23_na23)
  ...
  subroutine actual_jac(state)
    ...
    scratch_1 = 1.0d0*Y(jc12)*dens
    scratch_2 = state%rates(i_rate, k_c12_c12n_mg23)*state%rates(i_scor, k_c12_c12n_mg23)
    scratch_3 = scratch_1*scratch_2
    scratch_4 = state%rates(i_rate, k_c12_c12p_na23)*state%rates(i_scor, k_c12_c12p_na23)
    scratch_5 = scratch_1*scratch_4
    scratch_6 = state%rates(i_rate, k_c12_c12a_ne20)*state%rates(i_scor, k_c12_c12a_ne20)
    scratch_7 = scratch_1*scratch_6
    scratch_8 = 2.0d0*Y(jc12)*dens
    scratch_9 = state%rates(i_rate, k_ne23_na23)*state%rates(i_scor, k_ne23_na23)
    scratch_10 = state%rates(i_rate, k_na23_ne23)*state%rates(i_scor, k_na23_ne23)
    ...
    ! state%jac(j, i) = d(YDOT(j))/dY(i)
      ...
      state%jac(jc12,jc12) = ( &
        -scratch_2*scratch_8 - scratch_4*scratch_8 - scratch_6*scratch_8 &
         )
      ...
      state%jac(jne23,jhe4) = ( &
        0.0d0 &
         )

      state%jac(jne23,jc12) = ( &
        0.0d0 &
         )

      state%jac(jne23,jne20) = ( &
        0.0d0 &
         )

      state%jac(jne23,jne23) = ( &
        -scratch_9 &
         )

      state%jac(jne23,jna23) = ( &
        scratch_10 &
         )

      state%jac(jne23,jmg23) = ( &
        0.0d0 &
         )
